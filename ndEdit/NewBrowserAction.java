/*
 * $Id: NewBrowserAction.java,v 1.19 2005/10/18 23:44:53 oz Exp $
 *
...
public class NewBrowserAction extends NdEditAction {
    ...
    public NewBrowserAction(String menu, String text, Icon icon, ViewManager vm, NdEdit parent) {
        super(menu, text, icon, vm, parent);
    }

    public NewBrowserAction(String menu, String text, Icon icon, KeyStroke ks, ViewManager vm, NdEdit parent) {
   			...
   			if (directory != null && mFileName != null) {
			    	...
			    	try {
			    		...
			    		if (fileFormat == EPSConstants.ARGOINVENTORYFORMAT || fileFormat == EPSConstants.GTSPPINVENTORYFORMAT) {
							...
							try {
							    final SwingWorker worker = new SwingWorker() {
									public Object construct() {
										JProgressBar progressBar = new JProgressBar(0, 300);
										...
										jf.setLocation(x, y);
										jf.setTitle("Parsing inventory file...");
										jf.setVisible(true);
											...
											convertPtrDBToNdEditPC(ptrDB);
							          		...
							          		pointerCollection = new TuplePointerCollection(
							          				latArr1,
													latArr2,
													lonArr1,
													lonArr2,
										...
										if (pointerCollection != null) {
											// create a new frame
											final JFrame pcfr = new JFrame();
											pcfr.setSize(900, 700);
											pcfr.addWindowListener(new WindowAdapter() {
												public void windowClosing(WindowEvent e) {
													pcfr.setVisible(false);
													pcfr.dispose();
												}
											});

											pcfr.setTitle("NdEdit 2.0");

											// create new ndedit bean
											Properties props = new Properties();
											if (Constants.SUPPORT_DIR != null)
												props.put("ndedit.support.dir", Constants.SUPPORT_DIR);

											if (Constants.USE_METAL)
												props.put("ndedit.use.metal", "true");
											else
												props.put("ndedit.use.metal", "false");

											NdEdit nd = new NdEdit(pcfr, true, mParent.getQuitMode(), props);
											pcfr.getContentPane().add(nd);

											pcfr.validate();
											pcfr.invalidate();
											pcfr.setLocation(10, 10);
											pcfr.setVisible(true);

											nd.setDataTitle(mFileName);
											nd.setPointerCollection(pointerCollection, true);
											nd.getFrame().setTitle(nd.getFrame().getTitle() + ": " + mFileName);
											nd.getViewManager().invalidateAllViews();
										}
							...
							try {
							    final SwingWorker worker = new SwingWorker() {
									public Object construct() {
										...
										progressBar.setString("Parsing pointer file...");
										progressBar.setValue(0);
										progressBar.setStringPainted(true);
										progressBar.setIndeterminate(true);
										...
										jf.show();
						  	  			...
						  	  			try {
											EpicPtrFactory ptrfact = new EpicPtrFactory();
											EpicPtrs prtDB = ptrfact.createEpicPtrs(mInputFile, sortKeys, false, false);

											//turn this dbase into a ndedits's internal data structures
											convertPtrDBToNdEditPC(prtDB);
							          		pointerCollection = new TuplePointerCollection(
							          				latArr1,
													latArr2,
													lonArr1,
			    			...
			    			//create a pointer
					    	EpicPtr epPtr = new EpicPtr(fileFormat, "Bottle Import", "BOTTLE", "na", "na", -99, -99,
			                  new GeoDate(), -99, -99, mFileName, directory);
							...
							File nf = new File(directory, mFileName);
							try {
								// create a new frame
								final JFrame ncfr = new JFrame();
								ncfr.setSize(900, 700);
								ncfr.addWindowListener(new WindowAdapter() {
									public void windowClosing(WindowEvent e) {
										ncfr.setVisible(false);
										ncfr.dispose();
									}
								});

								ncfr.setTitle("NdEdit 2.0");

								// create new ndedit bean
								NdEdit nd = new NdEdit(ncfr, true, mParent.getQuitMode());
								ncfr.getContentPane().add(nd);

								ncfr.validate();
								ncfr.invalidate();
								ncfr.setLocation(10, 10);
								ncfr.setVisible(true);

								NdEditView tblView = new NdEditView(nd);
	...
	public void convertPtrDBToNdEditPC(EpicPtrs ptrDB) {
		...
		try {
			...
			if (ptrDB.getFormat() == EPSConstants.ARGOPTRS) {
	...
	public void convertDbaseToNdEditPC(EPSDbase epsDB) {
	    ...
	    catch (Exception ex) {System.out.println("threw testing for subdatabase");}
		...
		boolean latRange = false;
		boolean lonRange = false;
		boolean depthRange = false;
		...
		while ((dbItor != null && dbItor.hasNext()) || (sdbItor != null && sdbItor.hasNext())) {
			...
			for (int i=0; i<numLons; i++) {
				...
				// apply the longitude convention (it west longitude convert to east longitude)
				myLon *= lonMutiplier;
