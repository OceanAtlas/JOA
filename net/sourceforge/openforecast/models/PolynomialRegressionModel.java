...
public class PolynomialRegressionModel extends AbstractForecastingModel
{
	...
	{
		String retStr = "Single variable polynomial regression (" + independentVariable + ")";		
		return retStr;
	}
...
//		description += " with an equation of: y = "+coefficient[0];
  ...
  public Vector<TSModelTerm> getModelTerms() {
  	Vector<TSModelTerm> terms = new Vector<TSModelTerm>();
		...
		for ( int i=1; i<coefficient.length; i++ ) {
				TSModelTerm term = new TSModelTerm(coefficient[i], i, TSModelTermParameter.getParam(independentVariable));
				terms.add(term);
			}
