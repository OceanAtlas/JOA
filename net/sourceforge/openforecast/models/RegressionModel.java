...
import javaoceanatlas.ui.TSModelTerm;
import javaoceanatlas.ui.TSModelTermParameter;
import net.sourceforge.openforecast.ForecastingModel;
import net.sourceforge.openforecast.DataPoint;
import net.sourceforge.openforecast.DataSet;

/**
 * Implements a single variable linear regression model using the variable named
 * in the constructor as the independent variable. The cofficients of the
 * regression - the intercept and the slope - as well as the accuracy indicators
 * are determined from the data set passed to init.
 * 
 * <p>
 * Once initialized, this model can be applied to another data set using the
 * forecast method to forecast values of the dependent variable based on values
 * of the dependent variable (the one named in the constructor).
 * 
 * <p>
 * A single variable linear regression model essentially attempts to put a
...
public class RegressionModel extends AbstractForecastingModel {
	 ...
	 * independent variable. For a valid model to be constructed, you should call
	 * init and pass in a data set containing a series of data points involving
	 * the given independent variable.
	 * 
	 * @param independentVariable
	 *          the name of the independent variable to use in this model.
	 */
	 ...
	 * 
	 * <p>
	 * Additionally, the accuracy indicators are calculated based on this data
	 * set.
	 * 
	 * @param dataSet
	 ...
	 * 
	 * @param dataPoint
	 *          the data point for which a forecast value (for the dependent
	 *          variable) is required.
	 * @return the same data point passed in but with the dependent value updated
	 *         to contain the new forecast value.
	 * @throws ModelNotInitializedException
	 *           if forecast is called before the model has been initialized with
	 *           a call to init.
	 */
	...
	public String getForecastType() {
		String retStr = "Single variable regression (" + independentVariable + ")";		
		return retStr;
	}
	...
	public String toString() {
		return "y=" + intercept + (slope > 0.0 ? "+" : "") + slope + "*" + independentVariable;
	}

	public Vector<TSModelTerm> getModelTerms() {
		Vector<TSModelTerm> terms = new Vector<TSModelTerm>();

		TSModelTerm term = new TSModelTerm(slope, 1, TSModelTermParameter.getParam(independentVariable));
		terms.add(term);
