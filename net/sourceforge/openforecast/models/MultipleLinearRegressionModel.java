...
public class MultipleLinearRegressionModel extends AbstractForecastingModel
{
	...
	{
		varNames = dataSet.getIndependentVariables();
	...
	{
		String retStr = "Multiple variable linear regression (";
		for ( String varn : varNames) {
			retStr += varn + ",";
		}
	...
	{
		String desc = String.valueOf(intercept);
  ...
  public Vector<TSModelTerm> getModelTerms() {
  	...
  	Set coeffSet = coefficient.entrySet();
		...
		while ( it.hasNext() ) {
				Map.Entry entry = (Map.Entry)it.next();
				double coeff = ((Double)entry.getValue()).doubleValue();
				TSModelTerm term = new TSModelTerm(coeff, 1, TSModelTermParameter.getParam(entry.getKey().toString()));
				terms.add(term);
			}
