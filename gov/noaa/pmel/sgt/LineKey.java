/*
 * $Id: LineKey.java,v 1.19 2004/05/17 21:45:08 dwd Exp $
 *
 ...
 *
 * @author Donald Denbo
 * @version $Revision: 1.19 $, $Date: 2004/05/17 21:45:08 $
 * @since 1.0
**/
...
//    label.setLayer(layer_);
  ...
  if(!(rend instanceof LineCartesianRenderer))
      throw new IllegalArgumentException("Renderer is not a LineCartesianRenderer");
    removeLineRenderer((LineCartesianRenderer)rend);
  }
  /**
   * Remove a line from the LineKey.
   *
  ...
  public void draw(Graphics g) {
    ...
    //
    // protect from line_ and label_ being changed while
    // drawing key
    //
    Vector tempLine = new Vector(line_);
    Vector tempLabel = new Vector(label_);
    numLines = tempLine.size();
    ...
    col = 0;
    Object obj;
    Enumeration labelIt = tempLabel.elements();
    for(Enumeration lineIt = tempLine.elements(); lineIt.hasMoreElements();) {
      ...
      case LineAttribute.HEAVY:
        stroke_.drawHeavy(g, xout, yout, null, 2, attr, false);
        break;
      case LineAttribute.DASHED:
        stroke_.drawDashed(g, xout, yout, null, 2, attr, false);
        break;
      case LineAttribute.STROKE:
        stroke_.drawStroke(g, xout, yout, null, 2, attr, false);
        break;
