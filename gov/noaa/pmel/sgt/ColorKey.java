/*
 * $Id: ColorKey.java,v 1.24 2004/05/19 15:01:03 dwd Exp $
 *
 ...
 *
 * @author Donald Denbo
 * @version $Revision: 1.24 $, $Date: 2004/05/19 15:01:03 $
 * @since 1.0
 * @see Ruler
 * @see ColorMap
 * @see Layer
...
//    moveable_ = false;
  ...
  public void addGraph(CartesianRenderer rend, SGLabel label)
    ...
    if(rend instanceof GridCartesianRenderer) {
      GridAttribute ga = (GridAttribute) ((GridCartesianRenderer) rend).
          getAttribute();
      cm = ga.getColorMap();
    } else if(rend instanceof LineCartesianRenderer) {
      if(((SGTLine)((LineCartesianRenderer)rend).getData()).hasAssociatedData()) {
...
//        cm = la.getAssociatedDataCmap();
 ...
 //   addVectorGraph((VectorCartesianRenderer)rend, label);
   ...
   * @since 3.1
   */
  public void removeGraph(CartesianRenderer rend)
  throws IllegalArgumentException {
    // do nothing
  }
  /**
  ...
  void drawBar(Graphics g,
    ...
    if(orient_ == HORIZONTAL) {
      ...
      for(int i=yend; i <= yloc; i++) {
        g.setColor(cm_.getColor(sTrans.getTransU(layer_.getYDtoP(i))));
