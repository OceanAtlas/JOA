/*
 * $Id: DataGroupLayer.java,v 1.7 2004/04/08 23:10:16 sirott Exp $
 *
...
public class DataGroupLayer extends Layer {
  ...
  public void removeData(DataHolder dh, DataKey key) {
    ...
    Layer layer = null;
    for(int i=0; i < dataLayers_.size(); i++) {
      layer = (Layer)dataLayers_.get(i);
      rend = ((CartesianGraph)layer.getGraph()).getRenderer();
      data = rend.getData();
      if(data == dh.getData()) {
        dataLayers_.remove(layer);
        if(i == 0) setGraph(new CartesianGraph("Graph"));
        break;
      }
    }
    if(key != null) key.removeGraph(rend);
    update();
  }
   ...
   * @since 3.1
   */
  public void addData(DataHolder dh, DataKey key)
      throws DataTargetMismatchException {
    ...
    if(rend == null) {  // first data on DataGroupLayer
      ...
      xLabel_ = new SGLabel(null, "X Axis Label", label.toString(), new Point2D.Double(0.0, 0.0));
      ...
      yLabel_ = new SGLabel(null, "Y Axis Label", label.toString(), new Point2D.Double(0.0, 0.0));
  ...
  private SGLabel getLabel(SGTData data, DataKey key) {
    SGLabel lineTitle = new SGLabel(null, "foobar34", "", new Point2D.Double(0.0, 0.0)); //data.getKeyTitle();
    SGLabel zLabel = null;
    if(lineTitle == null) {
      if(data instanceof SGTGrid) {
        SGTMetaData zMeta = ((SGTGrid)data).getZMetaData();
        if(zMeta != null) {
          ...
          zLabel = new SGLabel(null, "Z Label", label.toString(), new Point2D.Double(0.0, 0.0));
        }
      }
      if(zLabel == null) {
        lineTitle = xLabel_;
  ...
  public void updateAxis(int dir, SoTRange range,
