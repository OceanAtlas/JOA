/*
 * $Id: Panel.java,v 1.11 2004/04/10 22:25:31 sirott Exp $
 *
 ...
 *
 * @author Donald Denbo
 * @version $Revision: 1.11 $, $Date: 2004/04/10 22:25:31 $
 * @since 3.0
...
// add or update labels to layer
    while(lbIter.hasNext()) {
      ...
      if(child == null) {
        sgl = new SGLabel(null, label.getId(),
                          label.getText(),
                          hgt,
                          loc,
                          SGLabel.BOTTOM,
...
//        labelLayer_.addChild(sgl);
        ...
        sgl.addPropertyChangeListener(label.getPropListener());   // For label move events
      } else {
        sgl = (SGLabel)child;
...
//        key.setBoundsP(legend.getBoundsP());
   ...
   * @since 3.1
   */
  public void addData(DataHolder dh) throws DataTargetMismatchException {
    DataGroupLayer agLayer = findDataGroupLayer(dh.getDataGroup().getId());
    DataKey key = null;
    Legend legend = dh.getLegend();
    if(legend != null) key = (DataKey)legendLayer_.findChild(legend.getId());

    agLayer.addData(dh, key);
  }
   ...
   * @since 3.1
   */
  public void removeData(DataHolder dh) {
    DataGroupLayer agLayer = findDataGroupLayer(dh.getDataGroup().getId());
    DataKey key = null;
    Legend legend = dh.getLegend();
    if(legend != null) key = (DataKey)legendLayer_.findChild(legend.getId());

    agLayer.removeData(dh, key);
  }
   ...
   * @since 3.1
   */
  public DataGroupLayer findLayer(DataHolder dh) {
    return findDataGroupLayer(dh.getDataGroup().getId());
  }
