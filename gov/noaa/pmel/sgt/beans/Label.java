/*
 * $Id: Label.java,v 1.6 2004/04/10 22:25:31 sirott Exp $
 *
 ...
 *
 * @author Donald Denbo
 * @version $Revision: 1.6 $, $Date: 2004/04/10 22:25:31 $
 * @since 3.0
 **/
public class Label implements Serializable {
  ...
  private LabelPropertyChangeListener proplistener_ = new LabelPropertyChangeListener();
...
//    }
  ...
  public LabelPropertyChangeListener getPropListener(){
    return proplistener_;
  }

  // Bug in Introspector causes NullPointerExceptions when Label implements
  // the PropertyChangeListener class. We get around this by delegating to the
  // LabelPropertyChangeListener class
  class LabelPropertyChangeListener implements PropertyChangeListener {
    public void propertyChange(PropertyChangeEvent evt){
      if (evt.getSource() instanceof SGLabel){
        SGLabel sglabel = (SGLabel)evt.getSource();
        String name = evt.getPropertyName();
        if (name.equals("location")){
          if (evt.getNewValue() instanceof Point){
            Point oldp = (Point) evt.getOldValue();
            Point newp = (Point) evt.getNewValue();
//            Layer layer = sglabel.getLayer();
//            double x = layer.getXDtoP(newp.x);
//            double y = layer.getYDtoP(newp.y);
//            setLocationP(new Point2D.Double(x,y));
          }
        }
        if (evt.getNewValue() instanceof Point2D.Double){
          Point2D.Double oldp = (Point2D.Double) evt.getOldValue();
          Point2D.Double newp = (Point2D.Double) evt.getNewValue();
          setLocationP(newp);
        }
      }
    }
