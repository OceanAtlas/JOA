/*
 * $Id: Page.java,v 1.17 2004/05/17 21:45:08 dwd Exp $
 *
 ...
 *
 * @author Donald Denbo
 * @version $Revision: 1.17 $, $Date: 2004/05/17 21:45:08 $
 * @since 3.0
 * @stereotype bean
 **/
...
//  transient private Vector panels_;
  ...
  public Page() {
    ...
    pane_.addMouseListener(new DefaultMouseAdapter(this));
  }
  ...
  public void propertyChange(PropertyChangeEvent evt) {
    ...
    if(source instanceof PanelModel) {
      ...
      if(obj instanceof DataHolder) {
        if(property.equals("addData") || property.equals("removeData")) {
          try {
            pane_.setBatch(true);
            ((DataHolder)obj).notifyPanel(evt);
...
//        panel.setBorder(new EtchedBorder());
  ...
  private void pageMousePressed(MouseEvent event) {
    if(event.isPopupTrigger()) System.out.println("Page.pageMousePressed(): isPopupTrigger()");
  ...
  public static class DefaultMouseAdapter extends MouseAdapter  {
    ...
    public void mousePressed(MouseEvent event) {
      if(!page.pane_.isMouseEventsEnabled()) return;
      page.pageMousePressed(event);
    }

    public void mouseClicked(MouseEvent event) {
      if(!page.pane_.isMouseEventsEnabled()) return;
      page.pageMouseClicked(event);
    }

    public void mouseReleased(MouseEvent event) {
      if(!page.pane_.isMouseEventsEnabled()) return;
      page.pageMouseReleased(event);
    }
  }
