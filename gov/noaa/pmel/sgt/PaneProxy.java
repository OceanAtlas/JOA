/*
 * $Id: PaneProxy.java,v 1.36 2004/06/04 21:06:10 sirott Exp $
 *
 ...
 *
 * @author Donald Denbo
 * @version $Revision: 1.36 $, $Date: 2004/06/04 21:06:10 $
 */
class PaneProxy {
  ...
  void draw() {
    ...
    ((Graphics2D)goff).scale(zoom_, zoom_);
...
//    System.out.println("PaneProxy.drawLayers("+getId()+"): print = " + printer_ + ", pane = " + pane_.getBounds());
 ...
 /*       if(printer_ && comps[i] instanceof Panel) {
  ...
  class ScaledMouseEvent{
    ...
    int getModifiers(){
      return e_.getModifiers();
    }
    int getX(){
      return (int)(e_.getX()/zoom_);
    }
    int getY() {
      return (int)(e_.getY()/zoom_);
    }
    boolean isShiftDown(){
      return e_.isShiftDown();
    }
    boolean isControlDown(){
      return e_.isControlDown();
    }
  }
  ...
  boolean processMouseMotionEvent(MouseEvent e) {
    ScaledMouseEvent event = new ScaledMouseEvent(e);
    boolean event_handled = false;
    if(!mouseEventsEnabled_) return event_handled;
...
//    if(in_move_ && !moved_ && selectedobject_ instanceof Selectable) {
  ...
  private Graphics getScaledGraphics(){
    Graphics2D g2 = null;
    if (pane_ != null){
      g2 = (Graphics2D)pane_.getGraphics();
      g2.scale(zoom_, zoom_);
    }
    return g2;
  }
