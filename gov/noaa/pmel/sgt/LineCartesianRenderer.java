/*
 * $Id: LineCartesianRenderer.java,v 1.20 2004/05/17 21:45:08 dwd Exp $
 *
 ...
 *
 * @author Donald Denbo
 * @version $Revision: 1.20 $, $Date: 2004/05/17 21:45:08 $
 * @since 1.0
 */
    ...
    line:
      for(count=first; count < size; count++) {
        if(xp[count] != Integer.MIN_VALUE && yp[count] != Integer.MIN_VALUE) {
          ...
          xout[nout] = xp[count];
          yout[nout] = yp[count];
          if(line.hasAssociatedData()) adata[nout] = assoc[count];
        } else if (nout >= 0) {
          break line;
        }
      }
      ...
      case LineAttribute.MARK:
        if(line.hasAssociatedData()) {
          drawColorMark(g, xout, yout, adata, lsize, attr);
        } else {
      ...
      case LineAttribute.HEAVY:
        stroke_.drawHeavy(g, xout, yout, adata, lsize, attr, hasAssoc);
        break;
      case LineAttribute.DASHED:
        stroke_.drawDashed(g, xout, yout, adata, lsize, attr, hasAssoc);
        break;
      case LineAttribute.STROKE:
        stroke_.drawStroke(g, xout, yout, adata, lsize, attr, hasAssoc);
        break;
      ...
      case LineAttribute.SOLID:
        drawColorLine(g, xout, yout, adata, lsize, attr, hasAssoc);
      }
    }
  }
