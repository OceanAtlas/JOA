/*
 * $Id: ContourLevels.java,v 1.21 2003/12/11 23:49:41 dwd Exp $
 *
 ...
 *
 * @author Donald Denbo
 * @version $Revision: 1.21 $, $Date: 2003/12/11 23:49:41 $
 * @since 2.0
 */
public class ContourLevels implements Cloneable {
   ...
   * @link aggregationByValue
   */
  static private ContourLineAttribute solid_ = new ContourLineAttribute(ContourLineAttribute.SOLID);

  /**
   * @label heavy
   * @link aggregationByValue
   */
  static private ContourLineAttribute heavy_ = new ContourLineAttribute(ContourLineAttribute.HEAVY);

  /**
   * @label dashed
   * @link aggregationByValue
   */
  static private ContourLineAttribute dashed_ = new ContourLineAttribute(ContourLineAttribute.DASHED);
  ...
  /**
   * Get the floor index for the value
   */
  public int getIndex(Double dval) {
    if(!sorted_) sort();
    int maxIndex = levels_.size() - 1;
    Double[] dlevels = (Double[])levels_.toArray(new Double[0]);
    double val = dval.doubleValue();
    double[] lev = new double[maxIndex+1];
    for(int i=0; i <= maxIndex; i++) {
      lev[i] = dlevels[i].doubleValue();
    }
    if(val <= lev[0]) return -1;
    if(val >= lev[maxIndex]) return maxIndex;
    for(int i=0; i < maxIndex; i++) {
      if(val >= lev[i] && val < lev[i+1]) {
        return i;
      }
    }
    return 0;
