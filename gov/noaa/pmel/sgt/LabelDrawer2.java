/*
 * $Id: LabelDrawer2.java,v 1.7 2004/04/08 23:10:16 sirott Exp $
 *
 ...
 *
 * @author Donald Denbo
 * @version $Revision: 1.7 $, $Date: 2004/04/08 23:10:16 $
 * @since 2.0
 */
public class LabelDrawer2 implements LabelDrawer, Cloneable {
  ...
  public LabelDrawer2(Component c, String lbl, double hgt, Point2D.Double loc, int valign, int halign) {
  	comp_ = c;
    label_ = lbl;
    height_ = hgt;
    porigin_ = loc;
...
//      }
  ...
  public void draw(Graphics g) /*throws LayerNotFoundException*/  {
    ...
    computeBoundsD(g);
    if(clr_ == null) {
      g.setColor(Color.white);//layer_.getPane().getComponent().getForeground());
    } else {
      g.setColor(clr_);
    }
    if(orient_ == SGLabel.HORIZONTAL) {
      ...
      //
      xs = (int)porigin_.x;//layer_.getXPtoD(porigin_.x);
      ys = (int)porigin_.y;//layer_.getYPtoD(porigin_.y);
      drawString(g, xs, ys);
    }
  }
  ...
  public void setLocation(Point loc) {
    ...
    computeBoundsD(comp_.getGraphics());
  ...
  public void setBounds(int x, int y, int width, int height) {
    ...
    if(dorigin_.x != xd || dorigin_.y != yd) {
      dorigin_.x = xd;
      dorigin_.y = yd;
      porigin_.x = xd;//layer_.getXDtoP(xd);
      porigin_.y = yd;//layer_.getYDtoP(yd);
    }
  }
...
//    System.out.println("LabelDrawer2('" + label_ + "'): TextLayout = " + tlayout);
    ...
    sascent = (int)tlayout.getAscent();
    //
    xd = (int)porigin_.x;//layer_.getXPtoD(porigin_.x);
    yd = (int)porigin_.y;//layer_.getYPtoD(porigin_.y);
    //
    // set device origin
    //
    dorigin_.x = xd;
    ...
    // compute pbounds
    //
    pbounds_.x = dbounds_.x;//layer_.getXDtoP(dbounds_.x);
    pbounds_.y = dbounds_.x;//layer_.getYDtoP(dbounds_.y);
    pbounds_.width = dbounds_.x + dbounds_.width - pbounds_.x;
    pbounds_.height = pbounds_.y - dbounds_.y + dbounds_.height;
    //System.out.println("LabelDrawer2 pbounds:" + pbounds_ + ":" + label_);
  }
