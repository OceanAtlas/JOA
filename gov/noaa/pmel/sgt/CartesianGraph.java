/*
 * $Id: CartesianGraph.java,v 1.21 2004/04/08 23:10:16 sirott Exp $
 *
 ...
 *
 * @author Donald Denbo
 * @version $Revision: 1.21 $, $Date: 2004/04/08 23:10:16 $
 * @since 1.0
 */
public class CartesianGraph extends Graph {
  ...
  public void setClip(SoTRange xr, SoTRange yr) {
    if(xr.isTime() || yr.isTime()) {
      ...
      setClip(xr.getStart().getDouble(), xr.getEnd().getDouble(),
              yr.getStart().getDouble(), yr.getEnd().getDouble());
    }
  }
  /**
  ...
  /**
   * Get the X axis at the given index
   * @param index
   * @return axis found
   */
  public Axis getXAxis(int index){
    return (Axis)xAxis_.get(index);
  }
  ...
  /**
   * Get the Y axis at the given index
   * @param index
   * @return axis found
   */
  public Axis getYAxis(int index){
    return (Axis)yAxis_.get(index);
  }
