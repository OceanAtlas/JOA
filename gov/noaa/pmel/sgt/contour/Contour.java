...
import gov.noaa.pmel.sgt.SGLabel; // import
...
/**
 * Contour constructs a set of <code>ContourLine</code> objects based on the
 * <code>ContourLevels</code>, <code>SGTGrid</code>, and mask supplied.
 * Used by <code>GridCartesianRenderer</code> for <code>GridAttribute</code>
 * types of CONTOUR.
 * 
 * @author D. W. Denbo
...
public class Contour implements PropertyChangeListener {
	...
	//
	private static double DSLAB = 175.0;
	 ...
	 */
	public Contour(Component c, SGTGrid grid, ContourLevels conLevels, int prec) {
		// cg_ = cg;
		parent_ = c;
		grid_ = grid;
	...
	/**
	 * Set a <code>SGTGrid</code> object to be used to mask the data grid. The Z
	 * values are used to determine the masking, values of NaN and non-zero are
	 * set as MISSING.
	 */
	public void setMask(SGTGrid mask) {
		if (mask_ == null || !mask_.equals(mask))
			upToDate_ = false;
		mask_ = mask;
	}
	...
	/**
	 * Return the <code>Enumeration</code> of a <code>Vector</code> containing
	 * the <code>ContourLine</code> objects.
	 */
	...
	public void generateContourLines() {
		...
		while (lenum.hasMoreElements()) {
			...
			for (ii = 0; ii < nx_; ii++) { /* 1001 */
				loop1000: for (jj = 0; jj < ny_; jj++) { /* 1000 */
					i = ii;
					j = jj;
					...
					try {
						cl.setAttributes(contourLevels_.getDefaultContourLineAttribute(), contourLevels_
						    .getContourLineAttribute(zc));
					}
					...
					// here's where we set the label attributes from JOA preferences
					// font, font size, format, etc...
					// Font f = cl.getContourLineAttribute().getLabelFont();
					String labelText = JOAFormulas.formatDouble(zc, mPrec, false);
					cl.getContourLineAttribute().setLabelText(labelText);
	...
	private void computeCorners(int i, int j, double zc) {
		...
		for (int lcorner = 0; lcorner < 4; lcorner++) { /* 620 */
			...
			kabov_[lcorner] = 10;
			if (((il + 1) * (nx_ - il) > 0) && ((jl + 1) * (ny_ - jl) > 0) && (!gridFlag_.isMissing(il, jl))) {
	 ...
	 * <code>ContourLineAttribute</code> generate the contour labels. Must be
	 * only invoked after generateConourLines().
	 */
	public void generateContourLabels(Graphics g) {
		...
		while (elem.hasMoreElements()) {
			...
			if (Debug.CONTOUR) {
				System.out.println("drawLabelContourLine: lev = " + lev + ", cl.getContourLineAttribute() = "
				    + cl.getContourLineAttribute());
			}
			...
			if (kmax <= 1 || !cattr.isLabelEnabled())
				continue;
			//
			// create SGLabel at a dummy location and no rotation
			//
			label = new SGLabel(parent_, "CLevel", cattr.getLabelText(), cattr.getLabelHeightP(),
			    new Point2D.Double(0.0, 0.0), SGLabel.BOTTOM, SGLabel.CENTER);
//			 System.out.println("label = " + label + ", cattr.getLabelText() = " +
//			 cattr.getLabelText());
			...
			width = width + hhgt;
			if (Debug.CONTOUR) {
				System.out.println("drawLabeledContourLine: hhgt,width = " + hhgt + ", " + width);
			}
			//
			px = xArrayP();
			py = yArrayP();
