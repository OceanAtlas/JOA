/*
 * $Id: ContourLine.java,v 1.17 2004/05/17 21:45:09 dwd Exp $
 *
 ...
 *
 * @author D. W. Denbo
 * @version $Revision: 1.17 $, $Date: 2004/05/17 21:45:09 $
 * @since 2.0
 * @see Contour
...
public class ContourLine extends Vector {
  private StrokeDrawer2 stroke_ = new StrokeDrawer2();
 ...
 // }
  ...
  public void draw(Graphics g) {
    ...
    while(cenum.hasMoreElements()) {
      ...
      label.setColor(attr_.getColor());
      label.setAngle(angle);
      label.setLocationP(new Point2D.Double(xlab+2, ylab+5));
      try {
        label.draw(g);
      } catch (Exception e) {
  ...
  private void drawLine(Graphics g, double[] x, double[] y,
    ...
    for(i=0, k=kstart; k <= kend; k++, i++) {
      xp[i] = (int)x[k];//cg_.getLayer().getXPtoD(x[k]);
      yp[i] = (int)y[k];//cg_.getLayer().getYPtoD(y[k]);			
    ...
    case LineAttribute.HEAVY:
      stroke_.drawHeavy(g, xp, yp, null, size, (LineAttribute)defaultAttr_, false);
      break;
    case LineAttribute.DASHED:
      stroke_.drawDashed(g, xp, yp, null, size, (LineAttribute)defaultAttr_, false);
      break;
    case LineAttribute.STROKE:
      stroke_.drawStroke(g, xp, yp, null, size, (LineAttribute)defaultAttr_, false);
      break;
    default:
  ...
  private void drawLineSegment(Graphics g, double x0, double y0,
    ...
    xp = new int[2];
    yp = new int[2];
    xp[0] = (int)x0;//cg_.getLayer().getXPtoD(x0);
    yp[0] = (int)y0;//cg_.getLayer().getYPtoD(y0);
    xp[1] = (int)x1;//cg_.getLayer().getXPtoD(x1);
    yp[1] = (int)y1;//cg_.getLayer().getYPtoD(y1);
    switch(defaultAttr_.getStyle()) {
    case LineAttribute.HIGHLIGHT:
      stroke_.drawHighlight(g, xp, yp, size, (LineAttribute)defaultAttr_);
      break;
    ...
    //if(cg_ != null) {
      ...
      for(int k=0; k <= kmax_; k++) {
        ...
        if(isXTime()) {
          ...
          xp[k] = time.getTime();//cg_.getXUtoP(time);
        } 
        else {
          xp[k] = pt.x;//cg_.getXUtoP(pt.x);
        }
      }
   // }
    return xp;
  ...
  public double[] getYArrayP() {
   ...
   // if(cg_ != null) {
      ...
      for(int k=0; k <= kmax_; k++) {
        ...
        if(isYTime()) {
          time = (new GeoDate(timeRef_)).increment(pt.x, GeoDate.DAYS);
          yp[k] = time.getTime();//cg_.getYUtoP(time);
        } 
        else {
          yp[k] = pt.y;//cg_.getYUtoP(pt.y);
        }
      }
    //}
    return yp;
