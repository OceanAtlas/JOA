...
public abstract class DbaseElement extends Component implements EPSConstants {
	//CompositeDbaseElement mParent;
	/**
	...
	*/
	protected Vector mDBEntries = null;
	/**
	...
	*/
	...
	public EPSVariable getEPSVariable(int varid) {
		for (int i=0; i<mVariables.size(); i++) {
  			EPSVariable var = (EPSVariable)mVariables.elementAt(i);
			if (var.getID() == varid)
				return var;
		}
		return null;
	...
	public Axis getAxis(String name) {
		...
		for (int i=0; i<mAxes.size(); i++) {
  			Axis ax = (Axis)mAxes.elementAt(i);
			if (ax.getName().equalsIgnoreCase(name))
				return ax;
			else if (ax.getName().indexOf(name) >= 0)
				return ax;
		}
		return null;
	}
	...
	* generic name (eg u) 
	...
	public EPSVariable addEmptyEPSVariable(int epickeyvarid) {
		...
		Key key = null;
		try {
			key = EPSProperties.epicKeyDB.findKey(epickeyvarid);
		}
		catch (Exception ex) {
		}
	...
	public String getUnlimitedDimension() {
  	...
  	*
   	...
   	**/	
	public Vector getMeasuredVariables() {
		Vector varCollector = new Vector(50);
		int numVars = 0;
		for (int i=0; i<this.mVariables.size(); i++) {
			EPSVariable epsVar = (EPSVariable)this.mVariables.elementAt(i);
			...
			for (int n=0; n<mAxes.size(); n++) {
				Axis ax = (Axis)mAxes.elementAt(n);
				
				if (ax.getTranslatedName().equals(name)) {
  	...
  	*
   	...
   	**/	
	public int getNumMeasuredVariables() {
		int numVars = 0;
		for (int i=0; i<this.mVariables.size(); i++) {
			EPSVariable epsVar = (EPSVariable)this.mVariables.elementAt(i);
	...
	public void writeNetCDF(File outFile) throws IOException, EPSVariableException {
		...
		for (int a=0; a<numAtts; a++) {
			EPSAttribute epAtt = (EPSAttribute)this.mAttributes.elementAt(a);
	...
	public void createSubEntries(int numEntries, String filename) {
		...
		mDBEntries = new Vector(numEntries);
	}
	...
	//CompositeDbaseElement getCompositeParent() {
	//	return mParent;
	//}
}