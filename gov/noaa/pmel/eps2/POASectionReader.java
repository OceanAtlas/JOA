...
/**
 * <code>POASectionReader</code> 
 * Concrete implementation of the EPSFileReader interface to read, parse, and save a POA Section file
 *
 * @author oz
 * @version 1.0
 *
 * @see EPSFileReader
 */
public class POASectionReader implements EPSFileReader, EPSConstants {
	/**
	...
	*/
	protected String mProgressStr = "Reading POA Binary Data...";
	
  	/**
   	* Construct a new <code>POASectionReader</code> with a Dbase amd file.
   	*
	...
	public POASectionReader(Dbase dname, File inFile, EpicPtr ep) {
		mOwnerDBase = dname;
		mFile = inFile;
  	...
  	/**
   	* Construct a new <code>POASectionReader</code> with a Dbase amd file and a prompt string.
   	*
   	* @param dname Dbase that this reader will fill in
   	* @param inFile Source section data file
   	*
 	* @see Dbase
   	*/
	public POASectionReader(Dbase dname, File inFile, EpicPtr ep, String progress) {
		mOwnerDBase = dname;
		mFile = inFile;
  	...
  	/**
   	* Parse the poa section file and fill in the Dbase.
   	*
   	* @return Success code
   	*/
	public void parse() throws Exception {
		//ProgressDialog progress = new ProgressDialog(fv, "Reading POA Profile Data...", Color.blue);
    	...
    	try {
	    	...
	    	for (int s=0; s<numSections; s++) {
				...
				for (int p=0; p<numVars; p++) {
					...
					// create new property
					tempProperties[p] = new JOAParameter(tempVar, "");
				...
				for (int c=0; c<numCasts; c++) {
					...
					int iYear = EPSConstants.MISSINGVALUE;
					int iMonth = EPSConstants.MISSINGVALUE;
					int iDay = EPSConstants.MISSINGVALUE;
					...
					if (!dateErr) {
						if (date != EPSConstants.MISSINGVALUE) {
							if (date < 0) {
					...
					ord++; 
					JOAStation sh = new JOAStation(ord, shipCode, stnNum, castNum, lat, 
						lon, numBottles, iYear, iMonth, iDay, EPSConstants.MISSINGVALUE,
						EPSConstants.MISSINGVALUE, bottomdbar, varFlag);
					sech.mStations.addElement(sh);
					sh.setType("POA BOTTLE");
    			...
    			for (int sc=start; sc<end; sc++) {
    				...
    				for (int b=0; b<sh.mNumBottles; b++) {
    					for (int v=0; v<sech.mNumVars; v++) {
    						...
    						int varVal = 0;
    						double dVarVal = varVal;
    						try {varVal = inData.readShort();} catch (IOException e) {System.out.println("IOException reading bottle"); e.printStackTrace();}
	    					bytesRead += 2;
	    					if (varVal != EPSConstants.MISSINGVALUE) {
								dVarVal = varVal * tempProperties[v].mActScale + 
									tempProperties[v].mActOrigin;
							}
							else
								dVarVal = EPSConstants.MISSINGVALUE;
				    ...
				    GeoDate[] ta = {new GeoDate(sh.mMonth, sh.mDay, sh.mYear, hour, min, secs, msec)};
					...
					for (int v=0; v<sech.mNumVars; v++) { 
					    ...
					    if (ffrmt != null) {
					    	epsVar.addAttribute(0, "FORTRAN_format", EPCHAR, ffrmt.length(), ffrmt);
				    		epsVar.setFrmt(ffrmt);
					    }
					    if (units != null && units.length() > 0) {
					    	epsVar.addAttribute(1, "units", EPCHAR, units.length(), units);
				    		epsVar.setUnits(units);
					    }
					    if (keyID >= 0)	{
					    	sarray[0] = (short)type;
					    	//epsVar.addAttribute(2, "type", EPSHORT, 1, sarray);
					    }
					    if (keyID >= 0) {
					    	sarray[0] = (short)keyID;
					    	epsVar.addAttribute(3, "epic_code", EPSHORT, 1, sarray);
					    }
